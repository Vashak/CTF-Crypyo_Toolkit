...
| **`rsa_key_selection.py`** | RSA | Signature Forgery (Smooth Primes) | Exploits insecure RSA factor input by generating smooth primes to solve the Discrete Logarithm Problem (DLP) using Pohlig-Hellman and CRT. |
...
| **`rsa_repeated_squaring.py`** | RSA | Non-invertible `e` (($e = 2^16$)) | Exploits ($e = 2^16$ by extracting 16 consecutive modular square roots through a quadratic residue expansion tree, operating entirely within $\mathbb{Z}_p$. |
